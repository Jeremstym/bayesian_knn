...
def constant_approximation_mc(y, X, beta, k, niter=100):
    def summation_given_y(y):
        ...
        for x_i, label_i in zip(X, y):
            ...
            knn_xi_idx = dist_x_i.argsort()[:k+1] # takes itself into account
            nearest_labels_to_i = y[knn_xi_idx[1:]]
            sum1 += np.sum([1 for label in nearest_labels_to_i if label == label_i])/k
        return sum1
    ...
    log_result = np.log(len(y))*2*mc_sum
...
def acceptance_threshold(y, X, theta_new, theta_old, k_new, k_old):
    ...
    threshold = (model_conditionnal(y, X, beta_new, k_new)\
                    *jacobian(theta_new))*prob_kold\
                        /(model_conditionnal(y, X, beta_old, k_new)\
                          *jacobian(theta_old)*prob_knew)

    return threshold

def metropolis_hastings(y, X, niter, burning=None):
    ...
    for iter in range(niter):
        ...
        u = nprd.random()
        acceptance = acceptance_threshold(y, X, theta_new, theta_old, k_new, k_old)
        if acceptance > u:
            k_old = k_new
            theta_old = theta_new

    beta_final = beta_max*np.exp(theta_old)/(1+np.exp(theta_old))
    k_final = k_old

    return beta_final, k_final 
